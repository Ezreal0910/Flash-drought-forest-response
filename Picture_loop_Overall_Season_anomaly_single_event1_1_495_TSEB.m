...
%                 plot(period, data_SIF, '-','Color', '#C5FB2E', 'LineWidth', 3);
            case 'T'
                data_T = readmatrix("E:\Data\Drought_response\Region\Huanan\Dinghushan_region\CI\T\Huanan_FD_T_mean.csv");
                data_T = data_T(:,1);
                data_T = data_T(:)';

                % 构建新的输出文件名
                output_file = sprintf('%s_495_TSEB_SM_T.svg', words{1});
