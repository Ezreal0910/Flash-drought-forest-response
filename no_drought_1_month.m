...
for t = 1 : 4
    ...
    if all(forest_extract(:) == 0)
        ...
        result_Flash_Drought_Events_spring_no_drought = [result_Flash_Drought_Events_spring_no_drought, leaped_Events2];
        ...
        result_Slow_Drought_Events_spring_no_drought = [result_Slow_Drought_Events_spring_no_drought, leaped_Events2];
        ...
        result_Flash_Drought_Events_summer_no_drought = [result_Flash_Drought_Events_summer_no_drought, leaped_Events2];
        ...
        result_Slow_Drought_Events_summer_no_drought = [result_Slow_Drought_Events_summer_no_drought, leaped_Events2];
        ...
        result_Flash_Drought_Events_autumn_no_drought = [result_Flash_Drought_Events_autumn_no_drought, leaped_Events2];
        ...
        result_Slow_Drought_Events_autumn_no_drought = [result_Slow_Drought_Events_autumn_no_drought, leaped_Events2];
        ...
        result_Flash_Drought_Events_winter_no_drought = [result_Flash_Drought_Events_winter_no_drought, leaped_Events2];
        ...
        result_Slow_Drought_Events_winter_no_drought = [result_Slow_Drought_Events_winter_no_drought, leaped_Events2];
    ...
    selected_many_FD_event_spring_no_drought = cell(3900*col,1);
    selected_many_FD_event_summer_no_drought = cell(3900*col,1);
    selected_many_FD_event_autumn_no_drought = cell(3900*col,1);
    selected_many_FD_event_winter_no_drought = cell(3900*col,1);
    ...
    for m = 1:numel(extracted_values_FD)
        ...
        first_column_spring_no_drought = [];
        first_column_summer_no_drought = [];
        first_column_autumn_no_drought = [];
        first_column_winter_no_drought = [];
        ...
        for f = 1:size(sorted_FD_Event, 1)
            ...
            if drought_date <= 104 || drought_date > 816 %  确保干旱事件在03-18年间，以寻找不干旱的年份做对比
                % 最小值的周期在两端，则跳过当前循环
                continue;
            end
            ...
            overlapping_count_FD = 0; % 记录重叠的次数
            ...
            for e2 = 1:numel(FD_Events.PixelIdxList)
                ...
                if ~isempty(intersect(FD_Event, indices))
                    % 有重叠，增加重叠次数
                    overlapping_count_FD = overlapping_count_FD + 1;
                    
                    % 将该FD_Event添加到overlapping_FD_Events
                    only_one_overlapping_FD = FD_Event;

                    % 如果重叠次数大于一个，直接进行下一次循环
                    if overlapping_count_FD > 1
                        break;
                    end
                end
            end
            ...
            if overlapping_count_FD == 1
                ...
                if ismember(drought_date, only_one_overlapping_FD)
                    ...
                    FD_drought_develop = drought_date - FD_drought_start;
                    FD_drought_recovery = FD_drought_end - drought_date;
                    ...
                    if isequal(start_season, end_season)
                        ...
                        switch start_season
                            case 'spring'
                                % 计算前后两年的周期
                                periods_spring = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_spring = struct('period', num2cell(periods_spring), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_spring)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_spring(i);
                                    indices = current_period - FD_drought_develop - 4 : current_period + FD_drought_recovery + 4; 
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e2_sp = 1:numel(FD_Events.PixelIdxList)
                                        FD_Event = FD_Events.PixelIdxList{e2_sp};
                                        
                                        % 检查FD_Event是否与indices有重叠
                                        if ~isempty(intersect(FD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_spring(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_spring(i).status == 0
                                        continue;
                                    end

                                    for e3_sp = 1:numel(SD_Events.PixelIdxList)
    
                                        SD_Event = SD_Events.PixelIdxList{e3_sp};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_spring.status] == 0);

                                % 如果所有周期的状态都为0，直接进行下一次循环
                                if all_zero
                                    continue; % 继续下一个 sorted_FD_Event
                                end
                                ...
                                for i = 1:numel(periods_spring)
                                    if results_spring(i).status ~= 0
                                        first_column_spring_no_drought(end+1) = periods_spring(i);
                                    end
                                end
                            case 'summer'
                                % 计算前后两年的周期
                                periods_summer = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_summer = struct('period', num2cell(periods_summer), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_summer)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_summer(i);
                                    indices = current_period - FD_drought_develop - 4 : current_period + FD_drought_recovery + 4; 
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e2_su = 1:numel(FD_Events.PixelIdxList)
                                        FD_Event = FD_Events.PixelIdxList{e2_su};
                                        
                                        % 检查FD_Event是否与indices有重叠
                                        if ~isempty(intersect(FD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_summer(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_summer(i).status == 0
                                        continue;
                                    end

                                    for e3_su = 1:numel(SD_Events.PixelIdxList)
    
                                        SD_Event = SD_Events.PixelIdxList{e3_su};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_summer.status] == 0);
                                ...
                                for i = 1:numel(periods_summer)
                                    if results_summer(i).status ~= 0
                                        first_column_summer_no_drought(end+1) = periods_summer(i);
                                    end
                                end
                            case 'autumn'
                                % 计算前后两年的周期
                                periods_autumn = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_autumn = struct('period', num2cell(periods_autumn), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_autumn)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_autumn(i);
                                    indices = current_period - FD_drought_develop - 4 : current_period + FD_drought_recovery + 4; 
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e2_au = 1:numel(FD_Events.PixelIdxList)
                                        FD_Event = FD_Events.PixelIdxList{e2_au};
                                        
                                        % 检查FD_Event是否与indices有重叠
                                        if ~isempty(intersect(FD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_autumn(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_autumn(i).status == 0
                                        continue;
                                    end

                                    for e3_au = 1:numel(SD_Events.PixelIdxList)
    
                                        SD_Event = SD_Events.PixelIdxList{e3_au};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_autumn.status] == 0);
                                ...
                                for i = 1:numel(periods_autumn)
                                    if results_autumn(i).status ~= 0
                                        first_column_autumn_no_drought(end+1) = periods_autumn(i);
                                    end
                                end
                            case 'winter'
                                % 计算前后两年的周期
                                periods_winter = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_winter = struct('period', num2cell(periods_winter), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_winter)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_winter(i);
                                    indices = current_period - FD_drought_develop - 4 : current_period + FD_drought_recovery + 4; 
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e2_wi = 1:numel(FD_Events.PixelIdxList)
                                        FD_Event = FD_Events.PixelIdxList{e2_wi};
                                        
                                        % 检查FD_Event是否与indices有重叠
                                        if ~isempty(intersect(FD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_winter(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_winter(i).status == 0
                                        continue;
                                    end

                                    for e3_wi = 1:numel(SD_Events.PixelIdxList)
    
                                        SD_Event = SD_Events.PixelIdxList{e3_wi};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_winter.status] == 0);
                                ...
                                for i = 1:numel(periods_winter)
                                    if results_winter(i).status ~= 0
                                        first_column_winter_no_drought(end+1) = periods_winter(i);
                                    end
                                end
                            otherwise
                                disp('Unknown season.');
        ...
        if ~isempty(first_column_spring)
            ...
            selected_many_FD_event_spring_no_drought{position_index} = first_column_spring_no_drought;
        end
        ...
        if ~isempty(first_column_summer)
            ...
            selected_many_FD_event_summer_no_drought{position_index} = first_column_summer_no_drought;
        end
        ...
        if ~isempty(first_column_autumn)
            ...
            selected_many_FD_event_autumn_no_drought{position_index} = first_column_autumn_no_drought;
        end
        ...
        if ~isempty(first_column_winter)
            ...
            selected_many_FD_event_winter_no_drought{position_index} = first_column_winter_no_drought;
        end
    end
    ...
    % spring_no_drought
    selected_many_FD_event_2D_spring_no_drought = reshape(selected_many_FD_event_spring_no_drought, 3900, col);
    result_Flash_Drought_Events_spring_no_drought = [result_Flash_Drought_Events_spring_no_drought, selected_many_FD_event_2D_spring_no_drought];
    ...
    % summer_no_drought
    selected_many_FD_event_2D_summer_no_drought = reshape(selected_many_FD_event_summer_no_drought, 3900, col);
    result_Flash_Drought_Events_summer_no_drought = [result_Flash_Drought_Events_summer_no_drought, selected_many_FD_event_2D_summer_no_drought];
    ...
    % autumn_no_drought
    selected_many_FD_event_2D_autumn_no_drought = reshape(selected_many_FD_event_autumn_no_drought, 3900, col);
    result_Flash_Drought_Events_autumn_no_drought = [result_Flash_Drought_Events_autumn_no_drought, selected_many_FD_event_2D_autumn_no_drought];
    ...
    % winter_no_drought
    selected_many_FD_event_2D_winter_no_drought = reshape(selected_many_FD_event_winter_no_drought, 3900, col);
    result_Flash_Drought_Events_winter_no_drought = [result_Flash_Drought_Events_winter_no_drought, selected_many_FD_event_2D_winter_no_drought];
    ...
    selected_many_SD_event_spring_no_drought = cell(3900*col,1);
    selected_many_SD_event_summer_no_drought = cell(3900*col,1);
    selected_many_SD_event_autumn_no_drought = cell(3900*col,1);
    selected_many_SD_event_winter_no_drought = cell(3900*col,1);
    ...
    for n = 1:numel(extracted_values_SD)
        ...
        first_column2_spring_no_drought = [];
        first_column2_summer_no_drought = [];
        first_column2_autumn_no_drought = [];
        first_column2_winter_no_drought = [];
        ...
        for s = 1:size(sorted_SD_Event, 1)
            ...
            if overlapping_count == 1
                ...
                if ismember(drought_date, only_one_overlapping)
                    ...
                    SD_drought_develop = drought_date - SD_drought_start;
                    SD_drought_recovery = SD_drought_end - drought_date;
                    ...
                    if isequal(start_season, end_season)
                        ...
                        switch start_season
                            case 'spring'
                                % 计算前后两年的周期
                                periods_spring_SD = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_spring_SD = struct('period', num2cell(periods_spring_SD), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_spring_SD)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_spring_SD(i);
                                    indices = current_period - SD_drought_develop - 4 : current_period + SD_drought_recovery + 4;
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e5_sp = 1:numel(SD_Events.PixelIdxList)
                                        SD_Event = SD_Events.PixelIdxList{e5_sp};
                                        
                                        % 检查SD_Event是否与indices有重叠
                                        if ~isempty(intersect(SD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_spring_SD(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_spring_SD(i).status == 0
                                        continue;
                                    end

                                    for e6_sp = 1:numel(FD_Events.PixelIdxList)
    
                                        FD_Event = FD_Events.PixelIdxList{e6_sp};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_spring_SD.status] == 0);

                                % 如果所有周期的状态都为0，直接进行下一次循环
                                if all_zero
                                    continue; % 继续下一个 sorted_SD_Event
                                end
                                ...
                                for i = 1:numel(periods_spring_SD)
                                    if results_spring_SD(i).status ~= 0
                                        first_column2_spring_no_drought(end+1) = periods_spring_SD(i);
                                    end
                                end
                            case 'summer'
                                % 计算前后两年的周期
                                periods_summer_SD = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_summer_SD = struct('period', num2cell(periods_summer_SD), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_summer_SD)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_summer_SD(i);
                                    indices = current_period - SD_drought_develop - 4 : current_period + SD_drought_recovery + 4;
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e5_su = 1:numel(SD_Events.PixelIdxList)
                                        SD_Event = SD_Events.PixelIdxList{e5_su};
                                        
                                        % 检查SD_Event是否与indices有重叠
                                        if ~isempty(intersect(SD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_summer_SD(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_summer_SD(i).status == 0
                                        continue;
                                    end

                                    for e6_su = 1:numel(FD_Events.PixelIdxList)
    
                                        FD_Event = FD_Events.PixelIdxList{e6_su};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_summer_SD.status] == 0);
                                ...
                                for i = 1:numel(periods_summer_SD)
                                    if results_summer_SD(i).status ~= 0
                                        first_column2_summer_no_drought(end+1) = periods_summer_SD(i);
                                    end
                                end
                            case 'autumn'
                                % 计算前后两年的周期
                                periods_autumn_SD = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_autumn_SD = struct('period', num2cell(periods_autumn_SD), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_autumn_SD)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_autumn_SD(i);
                                    indices = current_period - SD_drought_develop - 4 : current_period + SD_drought_recovery + 4;
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e5_au = 1:numel(SD_Events.PixelIdxList)
                                        SD_Event = SD_Events.PixelIdxList{e5_au};
                                        
                                        % 检查SD_Event是否与indices有重叠
                                        if ~isempty(intersect(SD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_autumn_SD(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_autumn_SD(i).status == 0
                                        continue;
                                    end
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_autumn_SD.status] == 0);
                                ...
                                for i = 1:numel(periods_autumn_SD)
                                    if results_autumn_SD(i).status ~= 0
                                        first_column2_autumn_no_drought(end+1) = periods_autumn_SD(i);
                                    end
                                end
                            case 'winter'
                                % 计算前后两年的周期
                                periods_winter_SD = [drought_date - 92, drought_date - 46, drought_date + 46, drought_date + 92];
                                
                                % 初始化结果标记的存储
                                results_winter_SD = struct('period', num2cell(periods_winter_SD), 'status', 1); % 默认无重叠为1

                                % 遍历每个周期
                                for i = 1:numel(periods_winter_SD)
                                    % 获取当前周期的前后12个周期的范围
                                    current_period = periods_winter_SD(i);
                                    indices = current_period - SD_drought_develop - 4 : current_period + SD_drought_recovery + 4;
                                    
                                    % 检查FD_Events中是否有事件与当前范围重叠
                                    for e5_wi = 1:numel(SD_Events.PixelIdxList)
                                        SD_Event = SD_Events.PixelIdxList{e5_wi};
                                        
                                        % 检查SD_Event是否与indices有重叠
                                        if ~isempty(intersect(SD_Event, indices))
                                            % 有重叠，设置标记为0
                                            results_winter_SD(i).status = 0;
                                            break;  % 发现重叠，直接进行下一次循环
                                        end
                                    end

                                    % 如果已经有重叠，则跳过SD_Events检查
                                    if results_winter_SD(i).status == 0
                                        continue;
                                    end

                                    for e6_wi = 1:numel(FD_Events.PixelIdxList)
    
                                        FD_Event = FD_Events.PixelIdxList{e6_wi};
                                ...
                                % 检查所有周期的状态
                                all_zero = all([results_winter_SD.status] == 0);
                                ...
                                for i = 1:numel(periods_winter_SD)
                                    if results_winter_SD(i).status ~= 0
                                        first_column2_winter_no_drought(end+1) = periods_winter_SD(i);
                                    end
                                end
                            otherwise
                                disp('Unknown season.');
        ...
        if ~isempty(first_column2_spring)
            ...
            selected_many_SD_event_spring_no_drought{position_index} = first_column2_spring_no_drought;
        end
        ...
        if ~isempty(first_column2_summer)
            ...
            selected_many_SD_event_summer_no_drought{position_index} = first_column2_summer_no_drought;
        end
        ...
        if ~isempty(first_column2_autumn)
            ...
            selected_many_SD_event_autumn_no_drought{position_index} = first_column2_autumn_no_drought;
        end
        ...
        if ~isempty(first_column2_winter)
            ...
            selected_many_SD_event_winter_no_drought{position_index} = first_column2_winter_no_drought;
        end
    end
    ...
    % spring_no_drought
    selected_many_SD_event_2D_spring_no_drought = reshape(selected_many_SD_event_spring_no_drought, 3900, col);
    result_Slow_Drought_Events_spring_no_drought = [result_Slow_Drought_Events_spring_no_drought, selected_many_SD_event_2D_spring_no_drought];
    ...
    % summer_no_drought
    selected_many_SD_event_2D_summer_no_drought = reshape(selected_many_SD_event_summer_no_drought, 3900, col);
    result_Slow_Drought_Events_summer_no_drought = [result_Slow_Drought_Events_summer_no_drought, selected_many_SD_event_2D_summer_no_drought];
    ...
    % autumn_no_drought
    selected_many_SD_event_2D_autumn_no_drought = reshape(selected_many_SD_event_autumn_no_drought, 3900, col);
    result_Slow_Drought_Events_autumn_no_drought = [result_Slow_Drought_Events_autumn_no_drought, selected_many_SD_event_2D_autumn_no_drought];
    ...
    % winter_no_drought
    selected_many_SD_event_2D_winter_no_drought = reshape(selected_many_SD_event_winter_no_drought, 3900, col);
    result_Slow_Drought_Events_winter_no_drought = [result_Slow_Drought_Events_winter_no_drought, selected_many_SD_event_2D_winter_no_drought];
...
save_path_FD_spring = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\spring";
save_path_FD_spring_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\spring\FD_spring_periods";
save_path_FD_spring_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\FD\spring";

output_filename_FD_spring1 = "result_FD_Events_spring_1-4";
...
output_filename_FD_spring1_start = "result_FD_Events_spring_start_1-4";
...
output_filename_FD_spring1_end = "result_FD_Events_spring_end_1-4";
...
output_filename_FD_spring2 = "result_Flash_Drought_Events_spring_1-4";
...
output_filename_FD_spring2_start = "result_Flash_Drought_Events_spring_start_1-4";
...
output_filename_FD_spring2_end = "result_Flash_Drought_Events_spring_end_1-4";
...
output_filename_FD_spring3 = "result_FD_Events_slow_spring_1-4";
...
output_filename_FD_spring3_start = "result_FD_Events_slow_spring_start_1-4";
...
output_filename_FD_spring3_end = "result_FD_Events_slow_spring_end_1-4";
...
output_filename_FD_spring4 = "result_Flash_Drought_Events_spring_no_drought_1-4";
full_path_FD_spring4 = fullfile(save_path_FD_spring_no_drought, output_filename_FD_spring4);
save(full_path_FD_spring4, 'result_Flash_Drought_Events_spring_no_drought', '-v7.3');

fprintf('已输出result_FD_Events_spring_1-4\n');

save_path_FD_summer = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\summer";
save_path_FD_summer_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\summer\FD_summer_periods";
save_path_FD_summer_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\FD\summer";

output_filename_FD_summer1 = "result_FD_Events_summer_1-4";
...
output_filename_FD_summer1_start = "result_FD_Events_summer_start_1-4";
...
output_filename_FD_summer1_end = "result_FD_Events_summer_end_1-4";
...
output_filename_FD_summer2 = "result_Flash_Drought_Events_summer_1-4";
...
output_filename_FD_summer2_start = "result_Flash_Drought_Events_summer_start_1-4";
...
output_filename_FD_summer2_end = "result_Flash_Drought_Events_summer_end_1-4";
...
output_filename_FD_summer3 = "result_FD_Events_slow_summer_1-4";
...
output_filename_FD_summer3_start = "result_FD_Events_slow_summer_start_1-4";
...
output_filename_FD_summer3_end = "result_FD_Events_slow_summer_end_1-4";
...
output_filename_FD_summer4 = "result_Flash_Drought_Events_summer_no_drought_1-4";
full_path_FD_summer4 = fullfile(save_path_FD_summer_no_drought, output_filename_FD_summer4);
save(full_path_FD_summer4, 'result_Flash_Drought_Events_summer_no_drought', '-v7.3');

fprintf('已输出result_FD_Events_summer_1-4\n');

save_path_FD_autumn = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\autumn";
save_path_FD_autumn_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\autumn\FD_autumn_periods";
save_path_FD_autumn_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\FD\autumn";

output_filename_FD_autumn1 = "result_FD_Events_autumn_1-4";
...
output_filename_FD_autumn1_start = "result_FD_Events_autumn_start_1-4";
...
output_filename_FD_autumn1_end = "result_FD_Events_autumn_end_1-4";
...
output_filename_FD_autumn2 = "result_Flash_Drought_Events_autumn_1-4";
...
output_filename_FD_autumn2_start = "result_Flash_Drought_Events_autumn_start_1-4";
...
output_filename_FD_autumn2_end = "result_Flash_Drought_Events_autumn_end_1-4";
...
output_filename_FD_autumn3 = "result_FD_Events_slow_autumn_1-4";
...
output_filename_FD_autumn3_start = "result_FD_Events_slow_autumn_start_1-4";
...
output_filename_FD_autumn3_end = "result_FD_Events_slow_autumn_end_1-4";
...
output_filename_FD_autumn4 = "result_Flash_Drought_Events_autumn_no_drought_1-4";
full_path_FD_autumn4 = fullfile(save_path_FD_autumn_no_drought, output_filename_FD_autumn4);
save(full_path_FD_autumn4, 'result_Flash_Drought_Events_autumn_no_drought', '-v7.3');

fprintf('已输出result_FD_Events_autumn_1-4\n');

save_path_FD_winter = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\winter";
save_path_FD_winter_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\FD\winter\FD_winter_periods";
save_path_FD_winter_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\FD\winter";

output_filename_FD_winter1 = "result_FD_Events_winter_1-4";
...
output_filename_FD_winter1_start = "result_FD_Events_winter_start_1-4";
...
output_filename_FD_winter1_end = "result_FD_Events_winter_end_1-4";
...
output_filename_FD_winter2 = "result_Flash_Drought_Events_winter_1-4";
...
output_filename_FD_winter2_start = "result_Flash_Drought_Events_winter_start_1-4";
...
output_filename_FD_winter2_end = "result_Flash_Drought_Events_winter_end_1-4";
...
output_filename_FD_winter3 = "result_FD_Events_slow_winter_1-4";
...
output_filename_FD_winter3_start = "result_FD_Events_slow_winter_start_1-4";
...
output_filename_FD_winter3_end = "result_FD_Events_slow_winter_end_1-4";
...
output_filename_FD_winter4 = "result_Flash_Drought_Events_winter_no_drought_1-4";
full_path_FD_winter4 = fullfile(save_path_FD_winter_no_drought, output_filename_FD_winter4);
save(full_path_FD_winter4, 'result_Flash_Drought_Events_winter_no_drought', '-v7.3');

fprintf('已输出result_FD_Events_winter_1-4\n');

% SD
save_path_SD_spring = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\spring";
save_path_SD_spring_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\spring\SD_spring_periods";
save_path_SD_spring_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\SD\spring";

output_filename_SD_spring1 = "result_SD_Events_spring_1-4";
...
output_filename_SD_spring1_start = "result_SD_Events_spring_start_1-4";
...
output_filename_SD_spring1_end = "result_SD_Events_spring_end_1-4";
...
output_filename_SD_spring2 = "result_Slow_Drought_Events_spring_1-4";
...
output_filename_SD_spring2_start = "result_Slow_Drought_Events_spring_start_1-4";
...
output_filename_SD_spring2_end = "result_Slow_Drought_Events_spring_end_1-4";
...
output_filename_SD_spring3 = "result_SD_Events_slow_spring_1-4";
...
output_filename_SD_spring3_start = "result_SD_Events_slow_spring_start_1-4";
...
output_filename_SD_spring3_end = "result_SD_Events_slow_spring_end_1-4";
...
output_filename_SD_spring4 = "result_Slow_Drought_Events_spring_no_drought_1-4";
full_path_SD_spring4 = fullfile(save_path_SD_spring_no_drought, output_filename_SD_spring4);
save(full_path_SD_spring4, 'result_Slow_Drought_Events_spring_no_drought', '-v7.3');

fprintf('已输出result_SD_Events_spring_1-4\n');

save_path_SD_summer = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\summer";
save_path_SD_summer_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\summer\SD_summer_periods";
save_path_SD_summer_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\SD\summer";

output_filename_SD_summer1 = "result_SD_Events_summer_1-4";
...
output_filename_SD_summer1_start = "result_SD_Events_summer_start_1-4";
...
output_filename_SD_summer1_end = "result_SD_Events_summer_end_1-4";
...
output_filename_SD_summer2 = "result_Slow_Drought_Events_summer_1-4";
...
output_filename_SD_summer2_start = "result_Slow_Drought_Events_summer_start_1-4";
...
output_filename_SD_summer2_end = "result_Slow_Drought_Events_summer_end_1-4";
...
output_filename_SD_summer3 = "result_SD_Events_slow_summer_1-4";
...
output_filename_SD_summer3_start = "result_SD_Events_slow_summer_start_1-4";
...
output_filename_SD_summer3_end = "result_SD_Events_slow_summer_end_1-4";
...
output_filename_SD_summer4 = "result_Slow_Drought_Events_summer_no_drought_1-4";
full_path_SD_summer4 = fullfile(save_path_SD_summer_no_drought, output_filename_SD_summer4);
save(full_path_SD_summer4, 'result_Slow_Drought_Events_summer_no_drought', '-v7.3');

fprintf('已输出result_SD_Events_summer_1-4\n');

save_path_SD_autumn = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\autumn";
save_path_SD_autumn_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\autumn\SD_autumn_periods";
save_path_SD_autumn_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\SD\autumn";

output_filename_SD_autumn1 = "result_SD_Events_autumn_1-4";
...
output_filename_SD_autumn1_start = "result_SD_Events_autumn_start_1-4";
...
output_filename_SD_autumn1_end = "result_SD_Events_autumn_end_1-4";
...
output_filename_SD_autumn2 = "result_Slow_Drought_Events_autumn_1-4";
...
output_filename_SD_autumn2_start = "result_Slow_Drought_Events_autumn_start_1-4";
...
output_filename_SD_autumn2_end = "result_Slow_Drought_Events_autumn_end_1-4";
...
output_filename_SD_autumn3 = "result_SD_Events_slow_autumn_1-4";
...
output_filename_SD_autumn3_start = "result_SD_Events_slow_autumn_start_1-4";
...
output_filename_SD_autumn3_end = "result_SD_Events_slow_autumn_end_1-4";
...
output_filename_SD_autumn4 = "result_Slow_Drought_Events_autumn_no_drought_1-4";
full_path_SD_autumn4 = fullfile(save_path_SD_autumn_no_drought, output_filename_SD_autumn4);
save(full_path_SD_autumn4, 'result_Slow_Drought_Events_autumn_no_drought', '-v7.3');

fprintf('已输出result_SD_Events_autumn_1-4\n');


save_path_SD_winter = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\winter";
save_path_SD_winter_periods = "L:\GZW\Drought_quantify\Season_N&P_1_month\Planted\SD\winter\SD_winter_periods";
save_path_SD_winter_no_drought = "L:\GZW\Drought_quantify\Season_N&P_no_drought_1_month\Planted\SD\winter";

output_filename_SD_winter1 = "result_SD_Events_winter_1-4";
...
output_filename_SD_winter1_start = "result_SD_Events_winter_start_1-4";
...
output_filename_SD_winter1_end = "result_SD_Events_winter_end_1-4";
...
output_filename_SD_winter2 = "result_Slow_Drought_Events_winter_1-4";
...
output_filename_SD_winter2_start = "result_Slow_Drought_Events_winter_start_1-4";
...
output_filename_SD_winter2_end = "result_Slow_Drought_Events_winter_end_1-4";
...
output_filename_SD_winter3 = "result_SD_Events_slow_winter_1-4";
...
output_filename_SD_winter3_start = "result_SD_Events_slow_winter_start_1-4";
...
output_filename_SD_winter3_end = "result_SD_Events_slow_winter_end_1-4";
...
output_filename_SD_winter4 = "result_Slow_Drought_Events_winter_no_drought_1-4";
full_path_SD_winter4 = fullfile(save_path_SD_winter_no_drought, output_filename_SD_winter4);
save(full_path_SD_winter4, 'result_Slow_Drought_Events_winter_no_drought', '-v7.3');

fprintf('已输出result_SD_Events_winter_1-4\n');
