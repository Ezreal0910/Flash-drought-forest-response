...
for season_idx = 2:numel(season_paths) 
    ...
    for m = 1:numel(selected_files)
        ...
        if strcmp(data_kind, 'GPP')
            ...
            figure('Units', 'pixels', 'Position', [750, 10, 800, 1000]);
            % 绘制蒸腾变化量随时间变化的曲线及其填充
            h1 = subplot(2, 1, 1);
            % 绘制干旱的情况
...
%             p_no_drought = plot(period, data_GPP_no_drought, '-','Color', [9/255, 91/255, 208/255], 'LineWidth', 1.5);
            ...
            % 计算光带的上下边界
            offset = (0.08*16)/16; % 定义偏移量，可以调整这个值来改变光带的宽度
            upper_bound = data_GPP_no_drought + offset;
            lower_bound = data_GPP_no_drought - offset;

%             % 第一层光带，透明度较高
%             fill([period, fliplr(period)], [upper_bound+(0.08*23)/16, fliplr(lower_bound-(0.08*23)/16)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.7);
%             % 第二层光带，透明度较低
...
%             plot(period, data_GPP_no_drought, '-','Color',[12/255, 114/255, 31/255], 'LineWidth', 2.5, 'HandleVisibility', 'off');
            ...
            if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                ...
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Productivity', 'Productivity Anomaly'}, 'Location', 'southwest','FontSize', 18, 'FontWeight', 'bold');
            end
            ...
            if ~isempty(tree_kind)
                ...
                switch season_file
                    ...
                    % 添加长度标注并在文本中添加短线
                    text(lush_period(1)-0.15, mean([cumulative_transpiration_lush_offset(1), cumulative_transpiration_lush_offset(end)]), ...
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        case 'Summer'
                        text(lush_period(end), cumulative_transpiration_lush_offset(end)+4, ...
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        case 'Summer'
                         text(lush_period(end), cumulative_transpiration_lush_offset(end)+1, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
            ...
            ax2.XAxis.Label.Position(2) = ax2.XAxis.Label.Position(2) - abs_length3*1.2;
...
%             
% %             % 绘制不干旱的情况
% %             plot(drought_period([1, end]), Interpolation_values_no_drought([1, end]), 'o', 'MarkerSize', 2, 'LineWidth', 2 ,...
% %                 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerFaceColor', [181/255, 18/255, 16/255]);
% 
%             p_no_drought = plot(period, data_NEP_no_drought, '-','Color',[23/255, 216/255, 15/255], 'LineWidth', 1.5);
%             
%             
% %             plot(drought_period, drought_NEP_rate, '-', 'Color', [181/255, 18/255, 16/255], 'LineWidth', 1.5); % 底部边界线
...
%             p_later = patch(xverts_drought, yverts_drought, [23/255, 216/255, 15/255], 'FaceAlpha', 0.8, 'EdgeColor', 'none');
            ...
            % 计算光带的上下边界
            offset = 0.08/4; % 定义偏移量，可以调整这个值来改变光带的宽度
            upper_bound = data_NEP_no_drought + offset;
            lower_bound = data_NEP_no_drought - offset;

            % 第一层光带，透明度较高
%             fill([period, fliplr(period)], [upper_bound+0.08/4, fliplr(lower_bound-0.08/4)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.7);
%             % 第二层光带，透明度较低
%             fill([period, fliplr(period)], [upper_bound, fliplr(lower_bound)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.9);
            ...
            if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Productivity Anomaly'}, 'Location', 'northwest','FontSize', 14);
            else
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Productivity Anomaly'}, 'Location', 'northeast','FontSize', 18, 'FontWeight', 'bold');
            end
            legend_handle.Box = 'off';
                ...
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        case 'Spring'
                            ...
                            xticks_custom2 = 0:1:26;      
                            ylim_custom2 = [-3 2];    
                            yticks_custom2 = -3:1:2;
                        case 'Summer'
                            xlim_custom2 = [0 26];        
                            xticks_custom2 = 0:1:26;      
                            ylim_custom2 = [0 12];    
                            yticks_custom2 = 0:3:12;
                        case 'Autumn'
                            xlim_custom2 = [0 26];        
                            xticks_custom2 = 0:1:26;      
                            ylim_custom2 = [-3 1];    
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        ...
                        % 添加箭头符号
                        plot(lush_period(1), cumulative_transpiration_lush_offset(end)-0.3, '^r', ...
                            'MarkerFaceColor', [23/255, 216/255, 15/255], 'MarkerEdgeColor', [23/255, 216/255, 15/255], 'MarkerSize', 5);  % 上箭头
                        plot(lush_period(1), cumulative_transpiration_lush_offset(1)+0.3, 'vr', ...
                        ...
                        % 添加长度标注
                        text(lush_period(1)-0.25, mean([cumulative_transpiration_lush_offset(1), cumulative_transpiration_lush_offset(end)])+0.2, ...
                            sprintf('%.2f', length_of_line), 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle', ...
                        ...
                        line([drought_period(end), drought_period(end)], [cumulative_transpiration_drought_offset(1), cumulative_transpiration_drought_offset(end)], 'Color', [181/255, 18/255, 16/255], 'LineWidth', 1.6);
                        % 添加箭头符号
                        plot(drought_period(end), cumulative_transpiration_drought_offset(end)-0.3, '^r', ...
                            'MarkerFaceColor', [181/255, 18/255, 16/255], 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerSize', 5);  % 上箭头
                        plot(drought_period(end), cumulative_transpiration_drought_offset(1)+0.3, 'vr', ...
                            'MarkerFaceColor', [181/255, 18/255, 16/255], 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerSize', 5);  % 下箭头
...
%                             'FontName', 'Times New Roman', 'FontSize', 18,'FontWeight', 'bold','Color', [181/255, 18/255, 16/255]);
                                            ...
                                            % 添加长度标注并在文本中添加短线
                        text(drought_period(end)+0.25, mean([cumulative_transpiration_drought_offset(1), cumulative_transpiration_drought_offset(end)])+0.15, ...
                            ['{\bf————}', num2str(round(length_of_line2, 2))], ...
                            'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', ...
                            'FontName', 'Times New Roman', 'FontSize', 18, 'FontWeight', 'bold', 'Color', [181/255, 18/255, 16/255]);
                    end
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        ...
                        line([drought_period(end), drought_period(end)], [cumulative_transpiration_drought_offset(1), cumulative_transpiration_drought_offset(end)], 'Color', [181/255, 18/255, 16/255], 'LineWidth', 1.6);
                        % 添加箭头符号
                        plot(drought_period(end), cumulative_transpiration_drought_offset(end)-0.2, '^r', ...
                            'MarkerFaceColor', [181/255, 18/255, 16/255], 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerSize', 5);  % 上箭头
                ...
                switch season_file
                    ...
                    line([lush_period(1), lush_period(1)], [cumulative_transpiration_lush_offset(1), cumulative_transpiration_lush_offset(end)], 'Color', [23/255, 216/255, 15/255], 'LineWidth', 1.6);
                    % 添加箭头符号
                    plot(lush_period(1), cumulative_transpiration_lush_offset(end)-0.2, '^r', ...
                        'MarkerFaceColor', [23/255, 216/255, 15/255], 'MarkerEdgeColor', [23/255, 216/255, 15/255], 'MarkerSize', 5);  % 上箭头
                    plot(lush_period(1), cumulative_transpiration_lush_offset(1)+0.3, 'vr', ...
                        'MarkerFaceColor', [23/255, 216/255, 15/255], 'MarkerEdgeColor', [23/255, 216/255, 15/255],'MarkerSize', 5);  % 下箭头
                    ...
                    plot(drought_period(end), cumulative_transpiration_drought_offset(end)-0.2, '^r', ...
                        'MarkerFaceColor', [181/255, 18/255, 16/255], 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerSize', 5);  % 上箭头
                    plot(drought_period(end), cumulative_transpiration_drought_offset(1)+0.2, 'vr', ...
                        'MarkerFaceColor', [181/255, 18/255, 16/255], 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerSize', 5);  % 下箭头
                    ...
                    % 添加长度标注并在文本中添加短线
                    text(drought_period(end)+0.15, mean([cumulative_transpiration_drought_offset(1), cumulative_transpiration_drought_offset(end)]), ...
                        ['{\bf————}', num2str(round(length_of_line2, 2))], ...
                        'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', ...
                        'FontName', 'Times New Roman', 'FontSize', 18, 'FontWeight', 'bold', 'Color', [181/255, 18/255, 16/255]);
                end
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        case 'Summer'
                            text(drought_period(1), cumulative_transpiration_drought_offset(1)+0.06, ...
                                ['{\bf ', num2str(round(cumulative_transpiration_drought_offset(1), 2)), '} \bf{——}'], ...
                                'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right', ...
                                'FontName', 'Times New Roman', 'FontSize', 20, 'FontWeight', 'bold');
                    end
            ...
            if ~isempty(tree_kind)
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        case 'Summer'
                        text(drought_period(end), cumulative_transpiration_drought_offset(end)-0.3, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_drought_offset(end), 2))], ...
                            'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                            'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [181/255, 18/255, 16/255]);
                    end
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        case 'Summer'
                        text(lush_period(end), cumulative_transpiration_lush_offset(end)+0.2, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        case 'Summer'
                         text(lush_period(end), cumulative_transpiration_lush_offset(end)-0.3, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'FontName', ...
                            'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [23/255, 216/255, 15/255]);
                    end
                ...
                switch season_file
                    case 'Summer'
                     text(lush_period(end), cumulative_transpiration_lush_offset(end)+0.15, ...
                        ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                        'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                        'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [23/255, 216/255, 15/255]);
                end
...
%             p_no_drought = plot(period, data_NPP_no_drought, '-','Color', [154/255, 104/255, 196/255], 'LineWidth', 1.5);
            ...
            % 计算光带的上下边界
            offset = (0.08*12)/16; % 定义偏移量，可以调整这个值来改变光带的宽度
            upper_bound = data_NPP_no_drought + offset;
            lower_bound = data_NPP_no_drought - offset;

            % 第一层光带，透明度较高
%             fill([period, fliplr(period)], [upper_bound+(0.08*12)/16, fliplr(lower_bound-(0.08*12)/16)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.7);
%             % 第二层光带，透明度较低
%             fill([period, fliplr(period)], [upper_bound, fliplr(lower_bound)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.9);
            ...
            if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Productivity', 'Productivity Anomaly'}, 'Location', 'northwest','FontSize', 14);
            else
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Productivity', 'Productivity Anomaly'}, 'Location', 'southwest','FontSize', 18, 'FontWeight', 'bold');
            end
            legend_handle.Box = 'off';
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        case 'Summer'
                        text(lush_period(end), cumulative_transpiration_lush_offset(end)+2, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        case 'Summer'
                         text(lush_period(end), cumulative_transpiration_lush_offset(end)+0.4, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'FontName', ...
                             'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [154/255, 104/255, 196/255]);
                    end
...
%             fill([period, fliplr(period)], [upper_bound, fliplr(lower_bound)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.9);
            ...
            if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'northwest','FontSize', 14);
            else
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'northwest','FontSize', 18, 'FontWeight', 'bold');
            end
            legend_handle.Box = 'off';
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        ...
                        plot(lush_period(1), cumulative_transpiration_lush_offset(1)+2, 'vr', ...
                        ...
                        line([drought_period(end), drought_period(end)], [cumulative_transpiration_drought_offset(1), cumulative_transpiration_drought_offset(end)], 'Color', [181/255, 18/255, 16/255], 'LineWidth', 1.6);
                        % 添加箭头符号
                        plot(drought_period(end), cumulative_transpiration_drought_offset(end)-2, '^r', ...
                            'MarkerFaceColor', [181/255, 18/255, 16/255], 'MarkerEdgeColor', [181/255, 18/255, 16/255], 'MarkerSize', 5);  % 上箭头
            ...
            if ~isempty(tree_kind)
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        case 'Summer'
                         text(lush_period(end), cumulative_transpiration_lush_offset(end)+0.5, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'FontName', ...
                             'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [255/255, 214/255, 0/255]);
                    end
...
%             p_no_drought = plot(period, data_Reco_no_drought, '-','Color', [255/255, 120/255, 13/255], 'LineWidth', 1.5);
            ...
            % 计算光带的上下边界
            offset = (0.08*22)/16; % 定义偏移量，可以调整这个值来改变光带的宽度
            upper_bound = data_Reco_no_drought + offset;
            lower_bound = data_Reco_no_drought - offset;

            % 第一层光带，透明度较高
%             fill([period, fliplr(period)], [upper_bound+(0.08*22)/16, fliplr(lower_bound-(0.08*22)/16)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.7);
%             % 第二层光带，透明度较低
%             fill([period, fliplr(period)], [upper_bound, fliplr(lower_bound)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.9);
            ...
            if isempty(tree_kind)
                if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                    legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'northwest','FontSize', 14);
                elseif strcmp(season_file, 'Summer')
                    legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'southwest','FontSize', 18, 'FontWeight', 'bold');
                elseif strcmp(season_file, 'Winter')
                    legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'northeast','FontSize', 14);
                end
            else
                if strcmp(tree_kind, 'Natural')
                    if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                        legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'northwest','FontSize', 14);
                    elseif strcmp(season_file, 'Summer')
                        legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'southwest','FontSize', 14);
                    elseif strcmp(season_file, 'Winter')
                        legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Respiration', 'Respiration Anomaly'}, 'Location', 'northeast','FontSize', 14);
                    end
                elseif strcmp(tree_kind, 'Planted')
...
%             yticks_custom = -1:2:5;
            ...
            % 计算光带的上下边界
            offset = (0.08*10)/16; % 定义偏移量，可以调整这个值来改变光带的宽度
            upper_bound = data_T_no_drought + offset;
            lower_bound = data_T_no_drought - offset;

%             % 第一层光带，透明度较高
%             fill([period, fliplr(period)], [upper_bound+(0.08*10)/16, fliplr(lower_bound-(0.08*10)/16)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.7);
%             % 第二层光带，透明度较低
%             fill([period, fliplr(period)], [upper_bound, fliplr(lower_bound)], [151/255, 253/255, 82/255], 'EdgeColor', 'none', 'FaceAlpha', 0.9);
...
%             highlight_color = [255/255, 186/255, 58/255];
            ...
            if strcmp(season_file, 'Spring') || strcmp(season_file, 'Autumn')
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Transpiration','Transpiration Anomaly'}, 'Location', 'northwest','FontSize', 14);
            else
                legend_handle = legend([p_star, p_drought, p_original, p_no_drought, p_later, p_fill], {'Start and end point', 'Drought', 'Original', 'No Drought', 'Cumulative Transpiration','Transpiration Anomaly'}, 'Location', 'southwest','FontSize', 18, 'FontWeight', 'bold');
            end
            ...
            if ~isempty(tree_kind)
                if strcmp(tree_kind,'Natural')
                    switch season_file
                        case 'Summer'
                        text(lush_period(end), cumulative_transpiration_lush_offset(end)+0.8, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                ...
                elseif strcmp(tree_kind,'Planted')
                    switch season_file
                        case 'Summer'
                         text(lush_period(end), cumulative_transpiration_lush_offset(end)+0.6, ...
                            ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                            'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                             'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [12/255, 114/255, 31/255]);
                    end
                ...
                switch season_file
                    case 'Summer'
                     text(lush_period(end), cumulative_transpiration_lush_offset(end)+1.2, ...
                        ['\bf{—} ', num2str(round(cumulative_transpiration_lush_offset(end), 2))], ...
                        'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontName', ...
                         'Times New Roman','FontSize', 20, 'FontWeight', 'bold', 'Color', [12/255, 114/255, 31/255]);
                end
