...
% 指定路径
path = 'E:\Data\站点数据_1217\鼎湖山\FD_495_4';
...
% 替换第五个元素为指定路径
% selected_files{5} = 'E:\Data\Drought_response\Region\Huanan\Dinghushan_region\CI\T\Huanan_FD_T_mean.csv';
...
for i = 1:length(selected_files_all)
    ...
    for j = 1:length(current_files)
        ...
        switch fourth_word
            case 'LST'
                data_LST = readmatrix("E:\Data\站点数据_1217\鼎湖山\FD_495_4\FD_495_LST_mean.csv");
                data_t2m = readmatrix("E:\Data\站点数据_1217\鼎湖山\FD_495_4\FD_495_t2m_mean.csv");

                data_T = data_LST ./ data_t2m;
                data_T = data_T(:)';

                % 构建新的输出文件名
                output_file = sprintf('%s_495_LST_t2m.svg', words{1});

                plot(period, data_T, '-','Color', '#FB6501', 'LineWidth', 3);
        end
    end

    period_points = readmatrix("E:\Data\站点数据_1217\鼎湖山\绘图数据\FD_495_periods.csv");
...
%     ylabel_right_str = '( SIF ) mWm⁻² nm⁻¹ sr⁻¹';
    ...
    % 设置 y 轴刻度，只显示部分刻度
    set(gca, 'YTick', [1.01, 1.02, 1.03, 1.04]);
    
    % 获取当前的 y 轴刻度
    yTicks = get(gca, 'YTick');
    
    % 将刻度四舍五入到一位小数，并转换成字符串格式
    formattedLabels = sprintf('%.2f\n', yTicks);
    ...
    for j = 1:length(current_files)
        ...
        switch fourth_word
            case 'LST'
                ...
                h_T = plot(period, data_T, '-','Color', '#FB6501', 'LineWidth', 3);
        end
    end
    ...
    ax = gca;
    ax.YAxis.LineWidth = 2; % 左侧 y 轴的线宽
    ax.YAxis.FontSize = 20; % 左侧 y 轴的字体大小
    ax.YAxis.FontWeight = 'bold'; % 左侧 y 轴字体加粗
...
%     y_relative3 = y_norm3 * ax_position(4);
    ...
    % 设置保存路径和文件名
    save_path = 'E:\Data\站点数据_1217\鼎湖山\绘图数据';
